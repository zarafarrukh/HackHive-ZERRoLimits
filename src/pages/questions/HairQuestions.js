...
const HairQuestions = () => {
  ...
  const handleGenerate = async () => {
    ...
    if (
      !answers.hairLength ||
      !answers.hairType ||
      !answers.hairConcerns ||
      !answers.washingFrequency ||
      !answers.goals
    ) {
    ...
    <div className="p-6">
      ...
      <div className="mb-4">
        ...
        <div className="flex gap-2">
          {["Long", "Medium", "Short"].map((length) => (
            <button
              key={length}
              className={`p-2 border rounded ${answers.hairLength === length ? "selected" : ""}`}
              onClick={() => handleSelect("hairLength", length)}
            >
              {length}
            </button>
          ))}
      ...
      <div className="mb-4">
        ...
        <div className="flex gap-2">
          {["Straight", "Wavy", "Curly", "Coily"].map((type) => (
            <button
              key={type}
              className={`p-2 border rounded ${answers.hairType === type ? "selected" : ""}`}
      ...
      <div className="mb-4">
        ...
        <div className="flex gap-2 flex-wrap">
          {["Frizz", "Hair Loss", "Dryness", "Split Ends", "Dandruff", "Oily Scalp", "Heat Damage"].map(
            (concern) => (
              <button
                key={concern}
                className={`p-2 border rounded ${answers.hairConcerns === concern ? "selected" : ""}`}
                onClick={() => handleSelect("hairConcerns", concern)}
              >
                {concern}
              </button>
            )
      ...
      <div className="mb-4">
        ...
        <div className="flex gap-2">
          {["1-2", "3-4", "5+", "Daily"].map((freq) => (
            <button
              key={freq}
              className={`p-2 border rounded ${answers.washingFrequency === freq ? "selected" : ""}`}
              onClick={() => handleSelect("washingFrequency", freq)}
            >
              {freq} times
            </button>
          ))}
      ...
      <div className="mb-4">
        ...
        <div className="flex gap-2 flex-wrap">
          {["More Volume", "Stronger Hair", "Less Breakage", "Shinier Hair", "Faster Growth"].map((goal) => (
            <button
              key={goal}
              className={`p-2 border rounded ${answers.goals === goal ? "selected" : ""}`}
              onClick={() => handleSelect("goals", goal)}
            >
              {goal}
            </button>
          ))}
